...
export default function Profile() {
  ...
  // Load user from AsyncStorage & server
  const loadUser = async () => {
    try {
      ...
      const res = await api.get(`/api/users/${parsedUser.UserId}`);
  ...
  const loadPosts = async () => {
    ...
    try {
      ...
      console.log("Error loading posts:", err?.response?.data || err.message);
    } finally {
      setLoadingPosts(false);
    }
  };

  // Refresh user data every time Profile page comes into focus
  useFocusEffect(
    useCallback(() => {
      loadUser();
    }, [])
